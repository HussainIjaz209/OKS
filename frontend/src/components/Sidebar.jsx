...
const Sidebar = ({ role, isOpen, onClose, isRestricted }) => {
  const { user } = useAuth()

  const getMenuItems = () => {
    // If restricted (teacher pending OR student with high fees), only show Dashboard, Profile, and Fees (for students)
    if (isRestricted) {
      if (role === 'student') {
        return [
    ...
    switch (role) {
      case 'student':
        return [
          ...
          { path: '/student/exams', icon: faFileAlt, label: 'Exams' },
          { path: '/student/settings', icon: faCog, label: 'Settings' }
        ]
      case 'teacher':
        // If teacher is pending/rejected (not approved), only show Profile
        // Assuming we'll pass status prop or use auth context inside. 
        // But for cleaner implementation, parent can pass filtered items or we check context.
        // Let's modify to use useAuth here or accept a status prop.
        // Since we are modifying Sidebar structure significantly, let's keep it simple:
        // logic moved to getMenuItems call.
        return [
          ...
          { path: '/teacher/evaluation', icon: faUserCheck, label: 'Student Evaluation' },
          { path: '/teacher/settings', icon: faCog, label: 'Settings' }
        ]
      case 'admin':
        return [
          ...
          { path: '/admin/id-cards', icon: faIdCard, label: 'ID Cards' },
